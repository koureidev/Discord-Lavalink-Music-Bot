...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("rewind")
    .setDescription("Rewinds the current song by a specified amount of time.")
  ...
  async execute(interaction, client) {
    ...
    if (!player.queue.current || !player.queue.current.info.isSeekable)
      return interaction.reply({
        content: "❌ The current song cannot be seeked backwards.",
        flags: MessageFlags.Ephemeral,
      });

    await interaction.deferReply();

    const secondsToRewind = interaction.options.getInteger("seconds");
    const currentTrack = player.queue.current;
    const trackDuration = currentTrack.info.duration;

    const currentPosition = getEstimatedPosition(player);
    let newPosition = currentPosition - secondsToRewind * 1000;
