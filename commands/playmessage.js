...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    if (!message.attachments || message.attachments.size === 0) {
      return interaction.editReply({
        content: "❌ This message does not contain any attached file.",
      });
    }

    const allowedExtensions = [".mp3", ".opus", ".ogg", ".wav", ".flac"];
    const attachment = message.attachments.find((att) =>
      allowedExtensions.some((ext) => att.name.toLowerCase().endsWith(ext))
    );

    if (!attachment) {
      return interaction.editReply({
        content: `❌ No compatible audio file found! Use: ${allowedExtensions.join(
          ", "
        )}`,
      });
    }
    ...
    try {
      ...
      console.error(
        "Error in playmessage (Attempt 1 - Direct Stream):",
      ...
      try {
        ...
        console.error(
          "Error in playmessage (Manual Download):",
          (downloadError &&
